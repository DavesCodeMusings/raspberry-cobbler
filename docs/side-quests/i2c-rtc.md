...
## Enabling I2C
The first to getting the clock to function is enabling the I2C bus it uses to communicate. This involves:

* Enabling I2C in _config.txt_
* Loading kernel modules for I2C
* Verifying the device address

I2C is not enabled by default. A line needs to be added to _config.txt_ telling the Pi to active the bus.
...
> Using the command `i2cdetect 1` will probe all the devices on I2C bus one, and should show your clock module address in a table. Mine was 0x68.
...
## Configuring _rcS_
So far everything is good. The date and time are correct. But everything will be back Jan 1, 1970 if we don't make sure the clock is available at boot. Adding the following lines to _/etc/init.d/rcS_ will do the trick.
